...
async def get_narrative(action_type, attacker_mention, target_mention):
    """Get a short, single-line narrative for game actions."""
    if action_type == 'kill':
        prompt = (
            "Write ONE short, funny kill message as a single sentence. "
            "Player P1 kills player P2. "
            "Return ONLY the sentence, no lists, no bullets, no explanation. "
            "Use 'P1' and 'P2' exactly once each."
        )
    elif action_type == 'rob':
        prompt = (
            "Write ONE short, funny rob message as a single sentence. "
            "Player P1 robs player P2. "
            "Return ONLY the sentence, no lists, no bullets, no explanation. "
            "Use 'P1' and 'P2' exactly once each."
        )
    else:
    ...
    # Fallback + trimming to avoid long walls of text if the model misbehaves
    if not res:
        text = f"P1 {action_type}s P2."
    else:
        # Take the first non-empty line
        first_line = next((l.strip() for l in res.splitlines() if l.strip()), "")
        text = first_line or f"P1 {action_type}s P2."
